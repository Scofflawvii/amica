...
export const Settings = ({ onClickClose }: { onClickClose: () => void }) => {
  ...
  const [chatbotBackend, setChatbotBackend] = useState(
    config("chatbot_backend"),
  );
  ...
  const [animationProcedural, setAnimationProcedural] = useState<boolean>(
    config("animation_procedural") === "true" ? true : false,
  );
  ...
  const [reasoningEngineEnabled, setReasoningEngineEnabled] = useState<boolean>(
    config("reasoning_engine_enabled") === "true" ? true : false,
  );
  ...
  const [externalApiEnabled, setExternalApiEnabled] = useState<boolean>(
    config("external_api_enabled") === "true" ? true : false,
  );
  ...
  const [levelLog, setLevelLog] = useState<boolean>(
    config("debug_log") === "false" ? false : true,
  );
  const [levelInfo, setLevelInfo] = useState<boolean>(
    config("debug_info") === "false" ? false : true,
  );
  const [levelWarn, setLevelWarn] = useState<boolean>(
    config("debug_warn") === "false" ? false : true,
  );
  const [debugGfx, setDebugGfx] = useState<boolean>(
    config("debug_gfx") === "true" ? true : false,
  );
  ...
  const [useWebGPU, setUseWebGPU] = useState<boolean>(
    config("use_webgpu") === "true" ? true : false,
  );
  ...
  useEffect(() => {
    ...
    if (
      amicaLifeEnabled &&
      ["echo", "moshi"].includes(config("chatbot_backend"))
    ) {
  ...
  }, [topMenuRef, backButtonRef, mainMenuRef, notificationsRef]);
      ...
      case "developer":
        return (
          <DeveloperPage
            ...
            levelLog={levelLog}
            setLevelLog={setLevelLog}
            levelInfo={levelInfo}
            setLevelInfo={setLevelInfo}
            levelWarn={levelWarn}
            setLevelWarn={setLevelWarn}
            debugGfx={debugGfx}
  ...
  return (
    <div className="text z-floating fixed top-0 left-0 max-h-full w-full overflow-y-auto text-left text-xs backdrop-blur">
      ...
      <div className="h-screen overflow-auto opacity-95 backdrop-blur">
        <div className="text-text1 mx-auto max-w-2xl py-16">
          <div className="mt-12">
      ...
      <div
        aria-live="assertive"
        className="pointer-events-none fixed inset-0 mt-2 flex items-end px-4 py-6 sm:items-start sm:p-6"
        ...
        <div className="flex w-full flex-col items-center space-y-4 sm:items-end">
          <Transition
            ...
            leaveTo="opacity-0">
            <div className="border-border/50 bg-surface-alt/90 pointer-events-auto w-full max-w-sm overflow-hidden rounded-lg border shadow-lg backdrop-blur">
              <div className="p-4">
                <div className="flex items-start">
                  ...
                  <div className="ml-4 flex flex-shrink-0">
                    <button
                      type="button"
                      className="text-muted inline-flex rounded-md hover:text-[hsl(var(--text))] focus:outline-none"
                      onClick={() => {
                        setShowNotification(false);
                      }}>
