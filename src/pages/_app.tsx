...
export default function App({ Component, pageProps }: AppProps) {
  ...
  useEffect(() => {
    ...
    methods.forEach((m) => {
      originals[m] =
        (console[m] as unknown as (...args: unknown[]) => void) ?? undefined;
      (console as unknown as Record<string, (...args: unknown[]) => void>)[m] =
        (...args: unknown[]) => {
          try {
            const mappedType = m === "log" ? "info" : m; // collapse log->info
            // error_handler_logs is ensured to be an array above
            w.error_handler_logs!.push({
              ts: Date.now(),
              type: mappedType,
              arguments: args,
            });
            if ((w.error_handler_logs!.length ?? 0) > cap) {
              w.error_handler_logs!.splice(
                0,
                (w.error_handler_logs!.length ?? 0) - cap,
              );
            }
          } catch {
