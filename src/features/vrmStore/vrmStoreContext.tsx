...
export const VrmStoreProvider = ({
  children,
}: PropsWithChildren<{}>): React.ReactElement => {
  const [isLoadingVrmList, setIsLoadingVrmList] = useState(true);
  const [loadedVrmList, vrmListDispatch] = useReducer(
    vrmStoreReducer,
    vrmInitList,
  );
  const vrmListAddFile = (file: File, viewer: Viewer) => {
    vrmListDispatch({
      ...
      callback: (callbackProp: AddItemCallbackType) => {
        viewer
          .loadVrm(callbackProp.url, (_progress: string) => {
...
export const useVrmStoreContext = () => {
  ...
  if (!context) {
    throw new Error(
      "useVrmStoreContext must be used inside the VrmStoreProvider",
    );
  }
