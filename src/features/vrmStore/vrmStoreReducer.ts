...
type AddItemAction = {
  ...
  itemFile: File;
  callback: (prop: AddItemCallbackType) => void;
};
type UpdateThumbAction = {
  type: VrmStoreActionType.updateVrmThumb;
  url: string;
  thumbBlob: Blob;
  vrmList: VrmData[];
  callback?: (list: VrmData[]) => void;
};
type SetListAction = {
  type: VrmStoreActionType.setVrmList;
  vrmList: VrmData[];
};
type LoadFromLocalStorageAction = {
...
const updateVrmThumb = (
  vrmList: VrmData[],
  action: UpdateThumbAction,
): VrmData[] => {
  ...
  if (vrm instanceof VrmData) {
    ...
    BlobToBase64(action.thumbBlob).then((data: string) => {
      ...
      if (action.callback)
        (action.callback as (list: VrmData[]) => void)(newVrmList);
    });
  }
  return vrmList;
};
...
const LoadFromLocalStorage = (
  action: LoadFromLocalStorageAction,
): VrmData[] => {
  vrmDataProvider
    .getItems()
...
const VrmDbModelToVrmData = async (
  vrmDbModel: VrmDbModel,
): Promise<VrmData> => {
  let thumbUrl: string;
  let vrmUrl: string;
  ...
  return new Promise((resolve) => {
    resolve(
      new VrmData(vrmDbModel.hash, vrmUrl, thumbUrl, vrmDbModel.saveType),
    );
  });
};
