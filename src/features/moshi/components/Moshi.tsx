  ...
  const handleWebSocketMessage = async (event: MessageEvent) => {
    ...
    if (tag === 1) {
      if (!decoderRef.current) {
        mlog.error(
          "Decoder reference is null or undefined. Skipping audio decoding.",
        );
        return;
      }
      ...
      try {
        ...
        if (!payload || payload.byteLength === 0) {
          mlog.warn("Empty or invalid payload received. Skipping.");
          return;
        }
        ...
        if (samplesDecoded > 0 && channelData?.[0]) {
          scheduleAudioPlayback(
            channelData[0],
            viewer.model?._lipSync?.audio.currentTime,
            viewer,
          );
        } else {
          mlog.warn(
            "Decoded audio data is invalid or empty. Skipping playback.",
          );
        }
      } catch (decodeError) {
        mlog.error("Error during audio decoding", decodeError);
      }
    }
