...
const tlog = logger.with({ subsystem: "tts", module: "coquiLocal" });
...
export async function coquiLocal(message: string) {
  ...
  try {
    ...
    tlog.error("Error in coquiLocal", error);
    throw error;
  }
}
...
export async function coquiLocalVoiceIdList() {
  try {
    ...
    const selectedValues =
      html
        .match(/value="([^"]+)" SELECTED/g)
        ?.map((match) => match.split('"')[1]) || [];

    return { list: selectedValues };
  } catch (error) {
    tlog.error("Error in coquiLocalVoiceIdList", error);
    throw error;
  }
}
